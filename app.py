...
function speakWelcome() {
  ...
  var preferred = voices.find(function(v){ return v.name.indexOf("Google US English") > -1 || v.name.indexOf("Samantha") > -1 || v.lang === "en-US"; });
...
""", height=80)
    ...
    BNI_TIPS = [
        ...
        "&#128161; <strong>BNI Tip:</strong> Your 60-second pitch should be crystal-clear — the easier you make it to refer you, the more referrals you receive!",
...
<div style="font-family:Arial,sans-serif; max-width:600px; margin:auto;">
  ...
  <div style="background:#f9f9f9; padding:24px; border-radius:0 0 10px 10px;">
    ...
    <p style="font-size:0.85em; color:#888;">
      ...
      helps small businesses save time and grow revenue. Visit <a href="https://mrceesai.com" style="color:#C8102E;">mrceesai.com</a>
    </p>
  </div>
</div>"""
  ...
  <div style="background:#f0fff4; padding:24px; border-radius:0 0 10px 10px; border:2px solid #27ae60;">
    ...
    if v_submitted:
        ...
        if errors:
            ...
            row = [timestamp, first_name.strip(), last_name.strip(), email.strip(),
            ...
            try:
                ...
                if ready_to_join in ["Ready to apply!", "Very interested"]:
                    send_hot_lead_alert(first_name.strip(), last_name.strip(), email.strip(),
                                        phone.strip(), business_name.strip(), final_industry,
                                        elevator_pitch.strip(), ready_to_join,
                                        gmail_user_v, gmail_pass_v, report_email_v)
            except KeyError:
                pass
            except Exception:
                pass
...
(function(){
  ...
  if(window.speechSynthesis){
    ...
    msg.rate=0.93; msg.pitch=1.1; window.speechSynthesis.speak(msg);
  }
})();
</script>
""", height=0)
...
# ════════════════════════════════════════════════════════════════════════════
#  TAB 2 — MEMBER ACTIVITY LOG
# ════════════════════════════════════════════════════════════════════════════
with tab_member:

    # Voice prompt for members
    st.components.v1.html("""
...
" onclick="speakMemberWelcome()" title="Click to hear member instructions">
  <span style="font-size:2em;">&#127942;</span>
  <div>
    <div style="font-weight:700; font-size:1.05em;">Member Activity Log — Record Your Contributions!</div>
    <div style="font-size:0.85em; opacity:0.88;">Click to hear how this works</div>
  </div>
</div>
<script>
function speakMemberWelcome() {
  ...
  window.speechSynthesis.cancel();
  var msg = new SpeechSynthesisUtterance(
    "Welcome, B N I member! Use this form to log your activity for today's meeting. " +
    "Record your name, and check off whether you are sharing a thank you for closed business, " +
    "passing a referral, or giving a testimonial. " +
    "All your contributions are saved automatically. Givers Gain!"
  );
...
<div class='member-intro-box'>
  <strong>Members — log your activity for today's meeting!</strong>
  Record your name and check off your contributions. This helps track chapter health,
  accountability, and shows leadership who is actively passing business.
</div>
""", unsafe_allow_html=True)
...
> Someone referred business TO you and it closed! You are thanking the member publicly for the referral that turned into revenue. Include the approximate value if you can — it motivates the whole chapter!

**&#128279; Referral Passed**
> You are passing a referral TO another member. You connected them with someone who could use their services. Include who you referred and who you referred them to.

**&#11088; Testimonial**
> You are giving a positive testimonial ABOUT another member's work, character, or results. This builds trust and credibility for that member in front of the chapter.
""")
    ...
    with st.form("member_form", clear_on_submit=True):
        ...
        if has_tyfcb:
            ...
            with col_t1:
                tyfcb_member = st.text_input("Which member referred the business? *", placeholder="Member's name", key="tyfcb_member")
            with col_t2:
                tyfcb_amount = st.text_input("Approximate value of closed business", placeholder="e.g. $2,500", key="tyfcb_amount")
            tyfcb_details = st.text_area("Brief description (what was the business?)", placeholder="e.g. John referred me to ABC Corp for a 3-month insurance review...", height=80, key="tyfcb_details")
        ...
        testimonial_text = ""
        if has_testimonial:
            testimonial_for = st.text_input("Testimonial for which member? *", placeholder="Member's name", key="test_for")
            testimonial_text = st.text_area(
                "Your testimonial *",
                placeholder="e.g. I just want to recognize Sarah Jones — she helped me redesign my entire website and the results have been incredible. My leads doubled in 30 days. I highly recommend her to anyone!",
                height=100, key="test_text"
            )

        st.markdown("## Additional Notes")
        member_notes = st.text_area("Anything else to share with the chapter? (optional)",
            placeholder="e.g. Announcements, 1-2-1 requests, upcoming events...", height=70, key="mnotes")
    ...
    if m_submitted:
        ...
        if not member_name.strip():
            m_errors.append("Your Full Name")
        if has_tyfcb and not tyfcb_member.strip():
            m_errors.append("Member who referred the business (TYFCB)")
        if has_tyfcb and not tyfcb_details.strip():
            m_errors.append("TYFCB description")
        if has_referral and not referral_details_list.strip():
            m_errors.append("Referral details")
        if has_testimonial and not testimonial_for.strip():
            m_errors.append("Testimonial recipient name")
        if has_testimonial and not testimonial_text.strip():
            m_errors.append("Testimonial text")
        ...
        if m_errors:
            ...
            try:
                ...
                # Try to get or create "Member Activity" sheet
                try:
                    ...
                    m_sheet.append_row([
                        ...
                        "Activity Types",
                        "Has TYFCB","TYFCB From Member","TYFCB Value","TYFCB Description",
                        "Has Referral","Referral Details",
                        "Has Testimonial","Testimonial For","Testimonial Text",
                        "Notes"
                    ])
            ...
            # Member voice celebration
            st.components.v1.html(f"""
<script>
if(window.speechSynthesis){{
  window.speechSynthesis.cancel();
  var actList = "{', '.join(activity_types)}";
  var msg = new SpeechSynthesisUtterance(
    "Great job, {member_name.strip()}! Your activity has been logged. " +
    "You recorded: " + actList + ". " +
...
<div class='member-success-card'>
  ...
  <table style='width:100%; font-size:0.95em; border-collapse:collapse; margin-top:12px;'>
    ...
    {"<tr><td style='padding:5px 8px; color:#555;'><strong>TYFCB From</strong></td><td style='padding:5px 8px;'>" + tyfcb_member.strip() + (f" — {tyfcb_amount.strip()}" if tyfcb_amount.strip() else "") + "</td></tr>" if has_tyfcb else ""}
  ...
  <p style='margin-top:14px; color:#555; font-size:0.92em;'>Your activity has been saved to the <strong>Member Activity</strong> tab of the Google Sheet. Keep giving, keep growing! &#127881;</p>
</div>
""", unsafe_allow_html=True)
